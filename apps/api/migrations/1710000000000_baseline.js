...
/**
 * Baseline 마이그레이션 — 기존 init.sql 스키마를 마이그레이션 체계로 편입.
 * 모두 IF NOT EXISTS/ON CONFLICT 라 기존 DB(이미 init.sql 로 생성됨)에서도 안전(멱등).
 * 이후 스키마 변경은 새 번호 마이그레이션으로만. init.sql 직접 수정 금지 (CONVENTIONS).
 *
 * 주의: init.sql 은 dev/prod 두 DB를 만들지만, 마이그레이션은 접속한 단일 DB에만 적용된다.
 */
exports.up = (pgm) => {
  pgm.sql(`
...
exports.down = (pgm) => {
  // 되돌리기: 앱 테이블만 제거(확장/pgboss 스키마는 보존).
  pgm.sql(`
